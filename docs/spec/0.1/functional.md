...
- Backwards compatible `weepin/` & `weepin update`
  Warning when loading a previous version.

- `-i` for `TemplateRI`s and other?
  For `TemplateRI`s `-i` will try to determine what files are available via the given protocol (is that even possible?).

- Searching, filtering based on types, owners and such?

- Some thing in env? What things in env?
